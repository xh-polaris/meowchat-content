...
type (
	...
	CatModel interface {
		catModel
		FindManyByCommunityId(ctx context.Context, communityId string, skip int64, count int64) ([]*db.Cat, int64, error)
		Search(ctx context.Context, communityId, keyword string, skip, count int64) ([]*db.Cat, int64, error)
	}
...
// NewCatModel returns a mapper for the mongo.
func NewCatModel(config *config.Config) CatModel {
	conn := monc.MustNewModel(config.Mongo.URL, config.Mongo.DB, CatCollectionName, config.Cache)
	...
	return &customCatModel{
		...
		es:              esClient,
		indexName:       fmt.Sprintf("%s.%s-alias", config.Mongo.DB, CatCollectionName),
	}
}
...
func (m *customCatModel) FindManyByCommunityId(ctx context.Context, communityId string, skip int64, count int64) ([]*db.Cat, int64, error) {
	data := make([]*db.Cat, 0, 20)
...
func (m *customCatModel) Search(ctx context.Context, communityId, keyword string, skip, count int64) ([]*db.Cat, int64, error) {
	search := m.es.Search
